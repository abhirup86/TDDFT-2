...
class TimeDependentHamiltonian():
    ...
    def __init__(self,calc,direction=[0,0,1]):
        ...
        for kpt in self.wfs.kpt_u:
            ...
            for n in range(self.nbands):
                ...
                den.append(self.pd.ifft(np.abs(kpt.psit_nG[n])**2,kpt.q))
        ...
        self.update_local_potential(calc.density.nt_sG[0])
        
        
    
    def update_local_potential(self,density):
        
        #tranform density from coarse to fine grids
        density=self.coarse_to_fine.apply(density)
        
        #calculte XC - potential
        VXC=xc.VLDAx(density)+xc.VLDAc(density)
        ...
        self.VXC=self.fine_to_coarse.apply(VXC)
    ...
    def init_potential(self):
        ...
        V=calc.hamiltonian.vbar.pd.zeros()
        calc.hamiltonian.vbar.add(V)
        self.Vloc=calc.hamiltonian.vbar.pd.ifft(V)
        
    def calculate_density(self,wfn):
        #calculation of density
        return np.einsum('qn,qmn,qmxyz->xyz',self.f_n,np.abs(wfn)**2,self.den_gs)
